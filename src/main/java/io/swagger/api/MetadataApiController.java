...
public class MetadataApiController implements MetadataApi {
    ...
    public ResponseEntity<SdkEmpty> setAnnotation(@ApiParam(value = "" ,required=true )  @Valid @RequestBody SdkKeyValue body) {
    ...
    public ResponseEntity<SdkEmpty> setLabel(@ApiParam(value = "" ,required=true )  @Valid @RequestBody SdkKeyValue body) {
        String accept = request.getHeader("Accept");
        if (accept != null && accept.contains("application/json")) {
            try {
                return new ResponseEntity<SdkEmpty>(objectMapper.readValue("{ }", SdkEmpty.class), HttpStatus.NOT_IMPLEMENTED);
